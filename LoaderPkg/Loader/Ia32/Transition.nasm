...
ASM_PFX(CallKernelThroughGateAsm):
    ...
    ; перевели старший байт в 0, чтобы
    ; отключить использование вирутальной памяти (paging)
    mov eax, cr0
    and eax, 0x7FFFFFFF
    mov cr0, eax
    ...
    ; LAB 2: Your code here:
    ; загружаем новую таблицу дескрипторов
    lgdt [GDT_DESCRIPTOR]
...
AsmWithOurGdt:
    ...
    ; включаем поддержку расширения физ. адреса (PAE) и глобальных страниц (PGE),
    ; необходимые для использования нового формата таблицы вирт. памяти
    mov eax, cr4
    or  eax, 0xA0 ; PAE - 5 bit, PGE - 7 bit. 
    mov cr4, eax
    ...
    ; LAB 2: Your code here:
    mov ecx, 0xC0000080 ; system MSR's register number
    rdmsr
    or eax, 0x900
    wrmsr
