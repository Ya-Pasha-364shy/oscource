...
/**
 * EFI_MEMORY_DESCRIPTOR
 * хранит информацию о всех регионах физической памяти.
 * Указатель на таблицу регионов памяти передается в uefi_lp
 * из UEFI, и для корректного определения объема доступной
 * памяти требуется полный проход по данной таблице.
*/
